...
const AnalyticsOverview = ({ data, dateRange }) => {
  ...
  if (loading) {
    return <LoadingState message={t('analytics:loadingData')} />;
  }

  // KPI cards using real data
  const kpiCards = [
    {
      title: t('analytics:assistantMessages'),
    ...
    <div className="space-y-8">
          ...
          <div key={card.title} className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 flex-1">
                ...
                <div className="mt-1">
                  ...
                  <div className="text-sm text-gray-500">{t('common:of', 'of')} {card.limit}</div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4">
              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-600">{t('analytics:usage')}</span>
              ...
              {card.percentage > 90 && (
                <p className="text-xs text-red-600 mt-1">{t('analytics:nearLimit')}</p>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">
          {t('analytics:currentPlan')} {subscriptionData?.plan_type?.charAt(0).toUpperCase() + subscriptionData?.plan_type?.slice(1) || 'Basic'}
        </h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-gray-500">{t('analytics:messages')}</span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-gray-500">{t('analytics:storage')}</span>
              <span className="text-sm text-gray-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg shadow p-6">
          <div className="flex items-start">
            ...
            <div className="ml-5">
              <h3 className="text-lg font-medium text-gray-900">{t('analytics:engagementAnalytics')}</h3>
        ...
        <div className="bg-gradient-to-br from-green-50 to-teal-50 rounded-lg shadow p-6">
          <div className="flex items-start">
            ...
            <div className="ml-5">
              <h3 className="text-lg font-medium text-gray-900">{t('analytics:knowledgeAnalytics')}</h3>
              <p className="mt-2 text-sm text-gray-600">
                {t('analytics:knowledgeInsights.performanceInsights')}
              </p>
              <span className="mt-3 inline-block text-sm font-medium text-green-600">
                {t('common:actions.inDevelopment')}
              </span>
            </div>
          </div>
        </div>
